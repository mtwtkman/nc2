...
pub(crate) struct Pallet {
    pub(crate) players: [Option<Player>; PALLET_HEIGHT_LIMIT],
}

impl Pallet {
    fn height(&self) -> usize {
        self.players.iter().filter(|x| x.is_some()).count()
    }
...
fn stack() {
    ...
    assert_eq!(
        &cell_has_two_players.pallet.players,
